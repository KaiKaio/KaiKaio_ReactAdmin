...
let musicReducer = (state:any, action:any) => {
  switch (action.type) {
    case 'getMusicList':
      return {
        ...state,
...
const Music: React.FC = () => {
  ...
  let handleDelete = (text: any) => {
    client.delete(text.delname).then((res: any) => { // Ali-oss 删除
      deleteMusic(text).then((res) => { // 数据库删除
        getMusicList().then((res) => { // 刷新列表
          dispatch({ type: 'getMusicList', payload: res})
        })
      }).catch((err) => {
        console.log(err)
  ...
  return (
    ...
    <div id="music">
      ...
      <Table<IMusicList> rowKey="_id" dataSource={ state.MusicList }>
        ...
        <Table.Column<IMusicList> key="title" title="标题" dataIndex="title" />
        <Table.Column<IMusicList> key="singer" title="歌手" dataIndex="singer" />
        <Table.Column<IMusicList> key="createtime" title="创建时间" dataIndex="createtime" />
